...
`crates/tinyone_core/Cargo.toml` and `crates/tinyone_ralloc/Cargo.toml` manifests plus the Python tools
...
bash scripts/consumer_compile.sh all              # C, C++, and Rust fixtures
bash scripts/consumer_smoke.sh                    # Unix native C smoke
bash scripts/consumer_smoke_msvc.sh               # Windows MSVC C smoke (requires cl.exe)
```
...
Add a `.to` file to `crates/tinyone_core/tests/language/pass/`, `fail_compile/`, or
...
1. **`crates/tinyone_core/src/builtins.rs`** — append a new entry after index 34 in
...
5. **`crates/tinyone_core/tests/stdlib_parity.rs`** — add at least one test covering the new
...
1. Add the builtin entry in `crates/tinyone_core/src/builtins.rs`.
2. Dispatch it through `crates/tinyone_core/src/runtime/builtins.rs`.
3. Implement it in `crates/tinyone_core/src/runtime/stdlib.rs`.
4. Test it through `crates/tinyone_core/tests/stdlib_parity.rs`.
...
Place fixtures under `crates/tinyone_core/tests/language/`:
...
- `crates/tinyone_core/tests/runtime_parity.rs` — language behavior, VM/JIT parity
- `crates/tinyone_core/tests/stdlib_parity.rs` — stdlib behavior
- `crates/tinyone_core/tests/abi_api_soundness.rs` — FFI contracts, artifact limits, verifier
...
5. Nullable `const char *` parameters must be annotated `/* nullable */` in the
   header.
6. Add a test in `crates/tinyone_core/tests/abi_api_soundness.rs` exercising the null-pointer
   and error cases.
