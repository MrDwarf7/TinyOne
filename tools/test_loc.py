    ...
    def test_audit_payload_contains_largest_files_and_warnings(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            stats = [
                ...
                loc_tool.count_file(root, "document.rst", ".rst", warn_large_bytes=20),
            ]
            ...
            self.assertEqual(payload["files"][0]["path"], "document.rst")
            self.assertEqual(payload["files"][0]["extension"], ".rst")
            self.assertIn("bytes", payload["files"][0])
            self.assertEqual(payload["warnings"][0]["path"], "document.rst")
