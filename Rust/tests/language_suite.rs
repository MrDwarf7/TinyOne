...
fn run_section(
    title: &str,
    root: &str,
    check: fn(&Fixture) -> FixtureResult,
...
fn check_runtime_fail_fixture(fixture: &Fixture) -> FixtureResult {
    ...
    match fixture.expected_error.as_deref() {
        Some(expected) => FixtureResult::pass(
            &fixture.path,
            format!(
                "vm+jit matched {expected:?}, inputs={}",
                fixture.inputs.len()
            ),
        ),
        None => FixtureResult::pass(
            &fixture.path,
            format!(
                "vm+jit failed at runtime as expected, inputs={}",
                fixture.inputs.len()
            ),
        ),
    }
...
fn temporary_importer_for(path: &Path) -> PathBuf {
    let stamp = SystemTime::now()
        ...
        .expect("system time")
        .as_nanos();
    let importer = std::env::temp_dir().join(format!(
        "tinyone-language-suite-import-{}-{stamp}.to",
        std::process::id()
    ));
    let target = path
        .canonicalize()
