...
pub fn promote_integer(lhs: TypeKind, rhs: TypeKind) -> Result<TypeKind> {
    if !lhs.is_integer() || !rhs.is_integer() {
        return Err(TinyOneError::runtime(
            "integer promotion requires integer operands",
        ));
    }
    if lhs == rhs {
        return Ok(lhs);
...
mod tests {
    ...
    fn integer_promotion_follows_spec_examples() {
        ...
        assert_eq!(
            promote_integer(TypeKind::I8, TypeKind::U8).unwrap(),
            TypeKind::I16
        );
        assert_eq!(
            promote_integer(TypeKind::U8, TypeKind::U16).unwrap(),
            TypeKind::U32
        );
        assert_eq!(
            promote_integer(TypeKind::I32, TypeKind::I64).unwrap(),
            TypeKind::I64
        );
    }
