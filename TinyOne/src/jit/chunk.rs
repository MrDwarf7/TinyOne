...
fn slot_immediate_compare_jump(
    code: &[Instr],
    pc: usize,
    branch_targets: &HashSet<usize>,
) -> Option<JitOp> {
...
fn array_len_slot_jump(
    ...
    Some(JitOp::ArrayLenSlotJumpIfZero(
        jit_operand(load.arg).ok()?,
        jit_operand(jump.arg).ok()?,
    ))
}
...
fn map_get_add_slots(code: &[Instr], pc: usize, branch_targets: &HashSet<usize>) -> Option<JitOp> {
    ...
    JitOp::map_get_add_slots(
        jit_operand(total.arg).ok()?,
...
mod tests {
    ...
    fn fuses_slot_immediate_compare_and_conditional_jump() {
        for compare in [Op::Lt, Op::Lte, Op::Gt, Op::Gte, Op::Eq, Op::Ne] {
            ...
            assert_eq!(
                ops[0].listing(),
                format!(
                    "slot.cmp.{}.i.jz 0 10 2",
                    compare.name().to_ascii_lowercase()
                )
            );
        }
    }
    ...
    fn fuses_slot_move_and_in_place_multiply_and_divide() {
        ...
        for (op, expected) in [
            (Op::Mul, JitOp::MulSlotInt(0, -3)),
            (Op::Div, JitOp::DivSlotInt(0, -3)),
        ] {
            let code = [
                Instr::new(Op::Load, 0, 0),
                Instr::new(Op::PushInt, -3, 0),
