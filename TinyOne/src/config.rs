...
impl ProjectConfig {
    ...
    pub(crate) fn load_for_entry(entry_file: &Path) -> Result<Self> {
        let entry_parent = entry_file.parent().ok_or_else(|| {
        ...
        for directory in entry_parent.ancestors() {
            ...
            let root = directory.canonicalize().map_err(|error| {
                TinyOneError::compile(format!("Config.toml project root error: {error}"))
            })?;
            let canonical_config = config_path.canonicalize().map_err(|error| {
                TinyOneError::compile(format!("Config.toml read error: {error}"))
            })?;
            if canonical_config.parent() != Some(root.as_path()) {
                return Err(TinyOneError::compile(
                    "Config.toml must not be a symlink outside its project root",
                ));
            }
            let bytes = read_limited(&canonical_config, MAX_CONFIG_BYTES, "Config.toml")?;
            let text = std::str::from_utf8(&bytes).map_err(|error| {
                TinyOneError::compile(format!("Config.toml must be UTF-8: {error}"))
            })?;
            let value: TomlValue = text.parse().map_err(|error| {
                TinyOneError::compile(format!("Config.toml parse error: {error}"))
            })?;
    ...
    fn parse(
        ...
        let require_configured_modules = rules
            .map(|table| {
                optional_bool(
                    table.get("require_configured_modules"),
                    "rules.require_configured_modules",
                )
            })
            .transpose()?
            .flatten()
    ...
    pub(crate) fn verify_module_signature(
        ...
        let module_directory = module_path.parent().ok_or_else(|| {
            TinyOneError::compile(format!(
                "Signed module {} has no containing directory",
                module_path.display()
            ))
        })?;
        let manifest_path =
            canonical_sidecar(module_directory, MODULE_MANIFEST_FILE, "module.toml")?;
        let signature_path =
            canonical_sidecar(module_directory, MODULE_SIGNATURE_FILE, "signature.toml")?;
        let manifest_bytes =
            read_limited(&manifest_path, MAX_MODULE_MANIFEST_BYTES, "Module manifest")?;
...
pub fn canonical_module_signature_payload(
...
pub fn module_signature_digest(
    module_manifest_toml: &str,
    signature_toml: &str,
) -> Result<[u8; 32]> {
...
fn parse_module_manifest(bytes: &[u8]) -> Result<ModuleManifest> {
    ...
    reject_unknown_fields(
        root,
        &["module", "purpose", "capabilities", "dependencies"],
        "module.toml",
    )?;
    ...
    Ok(ModuleManifest {
        ...
        network_outbound: capability_bool(
            network,
            "outbound",
            "module.toml capabilities.network.outbound",
        )?,
        network_listen: capability_bool(
            network,
            "listen",
            "module.toml capabilities.network.listen",
        )?,
        filesystem_read: capability_bool(
            filesystem,
            "read",
            "module.toml capabilities.filesystem.read",
        )?,
        filesystem_write: capability_bool(
            filesystem,
            "write",
            "module.toml capabilities.filesystem.write",
        )?,
        ...
        hardware_access: capability_bool(
            hardware,
            "access",
            "module.toml capabilities.hardware.access",
        )?,
        threads_allowed: capability_bool(
            threads,
            "allowed",
            "module.toml capabilities.threads.allowed",
        )?,
        unsafe_memory_allowed: capability_bool(
            memory,
            "unsafe",
            "module.toml capabilities.memory.unsafe",
        )?,
        linux_pipelines_allowed: capability_bool(
            pipelines,
            "linux",
            "module.toml capabilities.pipelines.linux",
        )?,
        dependencies,
    })
}
...
fn parse_signature_metadata(bytes: &[u8], signature_required: bool) -> Result<SignatureMetadata> {
    ...
    let source_hash = required_metadata_text(
    ...
    let compiler_version = required_metadata_text(
        artifact.get("compiler_version"),
        "signature.toml artifact.compiler_version",
        256,
    )?;
    let language_version = required_metadata_text(
        artifact.get("language_version"),
        "signature.toml artifact.language_version",
        64,
    )?;
    ...
    let algorithm = optional_metadata_text(
        signing.get("algorithm"),
        "signature.toml signing.algorithm",
        32,
    )?
    ...
    let authority_id = required_metadata_text(
        signing.get("authority"),
        "signature.toml signing.authority",
        128,
    )?;
    ...
    let policy_version = required_metadata_text(
        signing.get("policy_version"),
        "signature.toml signing.policy_version",
        64,
    )?;
    let issued_at =
        parse_unsigned_toml(signing.get("issued_at"), "signature.toml signing.issued_at")?;
    let expires_at = parse_unsigned_toml(
        signing.get("expires_at"),
        "signature.toml signing.expires_at",
    )?;
    let signing_record_id = required_metadata_text(
        signing.get("signing_record_id"),
        "signature.toml signing.signing_record_id",
        256,
    )?;
...
fn canonical_sidecar(module_directory: &Path, filename: &str, kind: &str) -> Result<PathBuf> {
    ...
    let canonical = path.canonicalize().map_err(|error| {
        TinyOneError::compile(format!("Signed module {kind} read error: {error}"))
    })?;
    if canonical.parent() != Some(module_directory)
        || canonical.file_name().is_none_or(|name| name != filename)
    {
        return Err(TinyOneError::compile(format!(
...
fn nested_capability_table<'a>(
    ...
    let table = optional_toml_table(
        capabilities.get(name),
        &format!("module.toml capabilities.{name}"),
    )?;
...
fn parse_environment_names(value: &TomlValue, field: &str) -> Result<Vec<String>> {
    let values = value.as_array().ok_or_else(|| {
        TinyOneError::compile(format!(
            "{field} must be an array of environment variable names"
        ))
    })?;
    ...
    for value in values {
        ...
        if !name.bytes().enumerate().all(|(index, byte)| {
            ...
            return Err(TinyOneError::compile(format!(
                "{field} must contain uppercase environment variable names"
            )));
        }
    ...
    for (name, version) in dependencies {
        ...
        parsed.insert(
            ...
            required_metadata_text(
                Some(version),
                &format!("module.toml dependencies.{name}"),
                256,
            )?,
        );
    }
...
fn parse_ed25519_signature(value: &TomlValue, field: &str) -> Result<Signature> {
    ...
    let signature_bytes = hex::decode(&signature_hex)
        .map_err(|_| TinyOneError::compile(format!("{field} must be hexadecimal")))?;
...
fn validate_sha256_hash(value: &str, field: &str) -> Result<()> {
    let encoded = value.strip_prefix("sha256:").ok_or_else(|| {
        TinyOneError::compile(format!(
            "{field} must use the canonical sha256:<lowercase-hex> form"
        ))
    })?;
    if encoded.len() != 64
        || !encoded
...
fn parse_signing(value: Option<&TomlValue>) -> Result<SigningPolicy> {
    ...
    reject_unknown_fields(
        signing,
        &["require_module_signatures", "authorities"],
        "signing",
    )?;
    let require_module_signatures = optional_bool(
        signing.get("require_module_signatures"),
        "signing.require_module_signatures",
    )?
    .unwrap_or(false);
...
fn central_roots() -> Result<HashMap<String, VerifyingKey>> {
    ...
    for item in roots.split(',').filter(|item| !item.is_empty()) {
        ...
        let public_key =
            parse_verifying_key(public_key_hex, "TINYONE_CENTRAL_AUTHORITY_ROOTS public key")?;
...
fn parse_authorities(
    ...
    for (index, item) in authorities.iter().enumerate() {
        let authority = item.as_table().ok_or_else(|| {
            TinyOneError::compile(format!("signing.authorities[{index}] must be a table"))
        })?;
        reject_unknown_fields(
            authority,
            &[
        ...
        let issuer_key = central_roots.get(issuer).ok_or_else(|| {
            TinyOneError::compile(format!(
                "Authority {id:?} names unknown central issuer {issuer:?}"
            ))
        })?;
        let public_key_hex = required_string(
            authority.get("public_key"),
            "signing.authorities.public_key",
        )?;
        let public_key = parse_verifying_key(public_key_hex, "signing.authorities.public_key")?;
        let not_before = parse_u64(
            authority.get("not_before"),
            "signing.authorities.not_before",
        )?;
        let expires = parse_u64(authority.get("expires"), "signing.authorities.expires")?;
        if expires <= not_before {
            return Err(TinyOneError::compile(format!(
                "Authority {id:?} expiry must be after not_before"
            )));
        }
        if now < not_before || now > expires {
            return Err(TinyOneError::compile(format!(
                "Authority {id:?} certificate is not currently valid"
            )));
        }
        let certificate_hex = required_string(
            authority.get("certificate"),
            "signing.authorities.certificate",
        )?;
        let certificate_bytes = hex::decode(certificate_hex).map_err(|_| {
            TinyOneError::compile("signing.authorities.certificate must be hexadecimal")
        })?;
        let certificate = Signature::from_slice(&certificate_bytes).map_err(|_| {
            TinyOneError::compile(
                "signing.authorities.certificate must be a 64-byte Ed25519 signature",
            )
        })?;
        ...
        issuer_key.verify(&digest, &certificate).map_err(|_| {
            TinyOneError::compile(format!(
                "Authority {id:?} certificate failed central-root verification"
            ))
        })?;
        if parsed
        ...
        {
            return Err(TinyOneError::compile(format!(
                "signing.authorities contains duplicate authority id {id:?}"
            )));
        }
    }
    Ok(parsed)
...
fn parse_verifying_key(value: &str, field: &str) -> Result<VerifyingKey> {
    let public_key_bytes = hex::decode(value)
        .map_err(|_| TinyOneError::compile(format!("{field} must be hexadecimal")))?;
    let public_key: [u8; 32] = public_key_bytes.try_into().map_err(|_| {
        TinyOneError::compile(format!("{field} must be a 32-byte Ed25519 public key"))
    })?;
...
fn parse_u64(value: Option<&TomlValue>, field: &str) -> Result<u64> {
    value
        ...
        .ok_or_else(|| {
            TinyOneError::compile(format!(
                "Config.toml field {field:?} must be a non-negative integer"
            ))
        })
}
...
fn validate_package(value: Option<&TomlValue>) -> Result<()> {
    ...
    reject_unknown_fields(
        package,
        &["name", "version", "description", "author", "authors"],
        "package",
    )?;
...
fn validate_module_path(path: &str, field: &str) -> Result<()> {
    ...
    {
        return Err(TinyOneError::compile(format!(
            "{field} must be a non-escaping TinyLang .to source path"
        )));
    }
    Ok(())
}
...
fn parse_capability_list(value: &TomlValue, field: &str) -> Result<ModuleCapabilities> {
    let values = value.as_array().ok_or_else(|| {
        TinyOneError::compile(format!("{field} must be an array of capability names"))
    })?;
    let names = values
        .iter()
        .map(|value| {
    ...
    value
        .map(|value| {
            value.as_table().ok_or_else(|| {
                TinyOneError::compile(format!("Config.toml field {field:?} must be a table"))
            })
        })
        .transpose()
}
...
fn required_string<'a>(value: Option<&'a TomlValue>, field: &str) -> Result<&'a str> {
    value.and_then(TomlValue::as_str).ok_or_else(|| {
        TinyOneError::compile(format!("Config.toml field {field:?} must be a string"))
    })
}
    ...
    value
        .map(|value| {
            value.as_bool().ok_or_else(|| {
                TinyOneError::compile(format!("Config.toml field {field:?} must be a boolean"))
            })
        })
        .transpose()
}
...
mod tests {
    ...
    fn central_root_certificate_delegates_company_module_signing() {
        ...
        let certificate = central.sign(
            &authority_certificate_digest(
                authority_id,
                company.verifying_key().as_bytes(),
                not_before,
                expires,
            )
        ...
        assert!(
            verified
                ...
                .allows(crate::ModuleCapability::Network)
        );
    ...
    fn direct_project_trusted_keys_are_not_a_signing_escape_hatch() {
        ...
        assert!(
            error.to_string().contains("signing.trusted_keys"),
            "{error}"
        );
    }
}
