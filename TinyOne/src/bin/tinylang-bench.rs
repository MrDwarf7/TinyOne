    ...
    fn new_graph(label: &str, module_count: usize) -> Self {
        ...
        for index in 0..module_count {
            main_source.push_str(&format!(
                "total = total + module_{index:03}.value({index})\n"
            ));
        }
                    ...
                    match command {
                        AllocatorWorkerCommand::Run => {
                            ...
                            for _ in 0..allocations_per_worker {
                                let mut buffer = RallocBuffer::try_new(allocation_bytes)
                                    .expect("allocate contended Ralloc buffer");
...
fn assess_current_evidence(
    ...
    if args.repeats < MIN_DECISION_REPEATS {
        rejections.push(format!(
            "{} repeats is below the decision minimum of {MIN_DECISION_REPEATS}",
            args.repeats
        ));
    }
...
fn runtime_pair(label: &str, source: String, iterations: u64) -> Vec<Benchmark> {
    ...
    let mut jit_program = JitProgram::compile_verified(&fixture.verified)
        .expect("attribution benchmark should compile");
    vec![
        bench(vm_name, iterations, move || {
            run_verified_mode(&vm_program, "vm", Vec::new())
        }),
        bench(jit_name, iterations, move || {
            run_compiled_jit(&mut jit_program, Vec::new())
        }),
    ]
}
...
fn collection_and_heap_benchmarks() -> Vec<Benchmark> {
    ...
    for phase in ["push_in_capacity", "capacity_growth", "pop", "clear"] {
        let size = if phase == "capacity_growth" {
            4_096
        } else {
            256
        };
    ...
    for phase in [
        ...
        benchmarks.extend(runtime_pair(
            &format!("heap_{phase}_256"),
            heap_phase_source(phase, 256),
            500,
        ));
    }
    benchmarks
...
fn module_graph_benchmarks() -> Vec<Benchmark> {
    ...
    for (label, module_count) in MODULE_GRAPH_SIZES {
        ...
        benchmarks.push(bench(uncached_name, iterations, {
            ...
            move || {
                black_box(
                    compile_file_verified(fixture.main()).expect("compile benchmark module graph"),
                );
            }
        }));
        ...
        let policy_bypasses = module_count <= 2
            ...
            || filesystem_context_for(&benchmark_fixture_root())
                == "wsl-mounted-windows-filesystem";
        ...
        benchmarks.push(bench(cached_name, iterations, {
            ...
            let (_, status) = compile_file_cached_verified_with_status(fixture.main())
                .expect("prime benchmark graph cache");
        ...
        if module_count == 16 && !policy_bypasses {
            benchmarks.push(bench("compiler.module_graph_medium_incremental", 200, {
                ...
                move || {
                    fs::write(
                        &changed_module,
                        format!("export fn value(input) {{ return input + {replacement} }}\n"),
                    )
                    .expect("change incremental benchmark module");
                    replacement ^= 1;
                    let (program, status) =
                        compile_file_cached_verified_with_status(fixture.main())
...
fn compile_cache_phase_benchmarks() -> Vec<Benchmark> {
    ...
    let (verified, status) = compile_file_cached_verified_with_status(fixture.main())
        .expect("prime cache phase fixture");
    ...
    vec![
        ...
        bench("compiler.cache_phase.metadata_decode", 20_000, {
            ...
            move || {
                black_box(
                    serde_json::from_slice::<JsonValue>(&bytes).expect("decode cache metadata"),
                );
            }
        }),
        ...
        bench("compiler.cache_phase.binary_decode_verify", 5_000, {
            ...
            move || {
                black_box(
                    VerifiedProgram::from_binary_artifact(&bytes)
                        .expect("decode verified cache artifact"),
                );
            }
        }),
...
fn jit_attribution_benchmarks() -> Vec<Benchmark> {
    ...
    let mut setup_program =
        JitProgram::compile_verified(&empty.verified).expect("compile JIT setup fixture");
    let dispatch = make_fixture(STRAIGHTLINE_SOURCE);
    let mut dispatch_program =
        JitProgram::compile_verified(&dispatch.verified).expect("compile JIT dispatch fixture");
    let calls = make_fixture(FUNCTION_SOURCE);
    let mut calls_program =
        JitProgram::compile_verified(&calls.verified).expect("compile JIT call fixture");
    let stack_reuse = make_fixture(STRAIGHTLINE_SOURCE);
    let mut stack_reuse_program = JitProgram::compile_verified(&stack_reuse.verified)
        .expect("compile JIT stack-reuse fixture");
    ...
    vec![
        bench("jit.execution_context_setup", 20_000, move || {
            run_compiled_jit(&mut setup_program, Vec::new())
        }),
        bench("jit.operand_stack_allocate_32", 100_000, || {
            black_box(Vec::<RuntimeValue>::with_capacity(32));
        }),
        bench("jit.operand_stack_reuse_32", 20_000, move || {
            run_compiled_jit(&mut stack_reuse_program, Vec::new())
        }),
        bench("jit.chunk_dispatch", 10_000, move || {
            run_compiled_jit(&mut dispatch_program, Vec::new())
        }),
        bench("jit.calls", 600, move || {
            run_compiled_jit(&mut calls_program, Vec::new())
        }),
...
fn build_benchmarks() -> Vec<Benchmark> {
    ...
    let mut benchmarks = vec![
        ...
        bench("allocator.ralloc_contention_4x32", 1_000, {
            ...
            move || {
                let contention =
                    contention.get_or_insert_with(|| AllocatorContention::new(4, 32, 64));
        ...
        bench("compiler.file_modules_cache_bypass", 2_000, {
            ...
            let (_, status) = compile_file_cached_verified_with_status(fixture.main())
                .expect("probe file compiler cache policy");
        ...
        bench("runtime.jit_straightline", 10_000, {
            let mut program = JitProgram::compile_verified(&straightline.verified)
                .expect("benchmark program should compile");
            move || run_compiled_jit(&mut program, Vec::new())
        }),
        bench("runtime.jit_loop_control", 2_000, {
            let mut program = JitProgram::compile_verified(&loop_fixture.verified)
                .expect("benchmark program should compile");
            move || run_compiled_jit(&mut program, Vec::new())
        }),
        bench("runtime.jit_hot_loop_4096_quickened", 100, {
            let mut program = JitProgram::compile_verified(&hot_loop.verified)
                .expect("benchmark program should compile");
            move || run_compiled_jit(&mut program, Vec::new())
        }),
        bench("runtime.jit_hot_loop_4096_no_quickening", 100, {
            ...
            let mut program =
                JitProgram::compile_verified_with_options(&hot_loop.verified, options)
                    .expect("benchmark program should compile");
            move || run_compiled_jit(&mut program, Vec::new())
        }),
        bench("runtime.jit_function_calls", 600, {
            let mut program = JitProgram::compile_verified(&functions.verified)
                .expect("benchmark program should compile");
            move || run_compiled_jit(&mut program, Vec::new())
        }),
        bench("runtime.jit_control_interrupts", 2_000, {
            let mut program = JitProgram::compile_verified(&interrupts.verified)
                .expect("benchmark program should compile");
            move || run_compiled_jit(&mut program, Vec::new())
        }),
        bench("runtime.jit_heap_structs", 1_000, {
            let mut program = JitProgram::compile_verified(&heap.verified)
                .expect("benchmark program should compile");
            move || run_compiled_jit(&mut program, Vec::new())
        }),
        bench("runtime.jit_builtin_heavy", 2_000, {
            let mut program = JitProgram::compile_verified(&builtins.verified)
                .expect("benchmark program should compile");
            move || run_compiled_jit(&mut program, Vec::new())
        }),
        bench("runtime.jit_vec_push_pop", 500, {
            let mut program = JitProgram::compile_verified(&vec_fixture.verified)
                .expect("benchmark program should compile");
            move || run_compiled_jit(&mut program, Vec::new())
        }),
        bench("runtime.jit_map_set_get", 500, {
            let mut program = JitProgram::compile_verified(&map_fixture.verified)
                .expect("benchmark program should compile");
            move || run_compiled_jit(&mut program, Vec::new())
        }),
        bench("runtime.jit_heap_churn", 500, {
            let mut program = JitProgram::compile_verified(&heap_churn.verified)
                .expect("benchmark program should compile");
            move || run_compiled_jit(&mut program, Vec::new())
        }),
    ...
    benchmarks.extend(runtime_pair(
        "slot_compare_immediate_4096",
        SLOT_COMPARE_SOURCE.to_string(),
        100,
    ));
    benchmarks.extend(runtime_pair(
        "slot_mul_immediate_4096",
        SLOT_MUL_SOURCE.to_string(),
        100,
    ));
    benchmarks.extend(runtime_pair(
        "slot_div_immediate_4096",
        SLOT_DIV_SOURCE.to_string(),
        100,
    ));
...
fn print_table(results: &[BenchmarkResult]) {
    ...
    for result in results {
        ...
        let flag = if result.cv_pct() > cv_limit {
            " !"
        } else {
            "  "
        };
    ...
    if results
        .iter()
        .any(|result| result.cv_pct() > cv_limit_for(result.name))
    {
        println!(
    ...
    document
        ...
        .ok_or_else(|| {
            TinyOneError::Runtime(
                "Baseline must be a legacy JSON list or an object with a benchmarks list"
                    .to_string(),
            )
        })
}
...
fn assess_saved_evidence(document: &JsonValue, items: &[JsonValue]) -> EvidenceQuality {
    ...
    let Some(metadata) = document.get("metadata") else {
        return EvidenceQuality::rejected(vec![
            "legacy baseline has no measurement metadata".to_string(),
        ]);
    };
    let Some(options) = metadata.get("benchmark_options") else {
        return EvidenceQuality::rejected(vec![
            "baseline has no benchmark measurement options".to_string(),
        ]);
    };
    if options
        .get("correctness_checked")
    ...
    if repeats.is_none_or(|repeats| repeats < MIN_DECISION_REPEATS as u64) {
        rejections.push(format!(
            "baseline repeats are below the decision minimum of {MIN_DECISION_REPEATS}"
        ));
    }
    ...
    for item in items {
        ...
        if cv_pct > limit {
            rejections.push(format!(
                "baseline {name} has {cv_pct:.2}% CV, above its {limit:.0}% limit"
            ));
        }
    }
    if rejections.is_empty() {
...
fn verify_priority_3_gate(
    ...
    for (name, label) in PRIORITY_3_ROWS {
        ...
        let cpu_improvement = metric_improvement_pct(
            previous,
            "best_cpu_time_per_iter_ns",
            current.best_per_iter_cpu_ns(),
        );
        let cycle_improvement = metric_improvement_pct(
            previous,
            "best_cycles_per_iter",
            current.best_per_iter_cycles(),
        );
        ...
        let metric = |name: &str, improvement: Option<f64>| match improvement {
    ...
    if failures.is_empty() {
        ...
        Err(TinyOneError::Runtime(format!(
            "Priority 3 gate failed: {}",
            failures.join("; ")
        )))
    }
}

fn verify_priority_5_gate(
    results: &[BenchmarkResult],
    baseline: &[JsonValue],
) -> Result<(), TinyOneError> {
    let mut failures = Vec::new();
    for (name, label) in PRIORITY_5_GUARDRAIL_ROWS {
        ...
        let primary_metrics = [
            (
                ...
                metric_regression_pct(
                    previous,
                    "mean_cpu_time_per_iter_ns",
                    current.mean_per_iter_cpu_ns(),
                ),
            ),
            (
                "cycles",
                metric_regression_pct(
                    previous,
                    "mean_cycles_per_iter",
                    current.mean_per_iter_cycles(),
                ),
            ),
        ];
        ...
        if !checked_primary_metric {
            let wall_regression = metric_regression_pct(
                previous,
                "mean_per_iter_ns",
                Some(current.mean_per_iter_ns()),
            );
    ...
    if failures.is_empty() {
        ...
        Err(TinyOneError::Runtime(format!(
            "Priority 5 gate failed: {}",
            failures.join("; ")
        )))
    }
}

fn require_priority_5_baseline_compatibility(
    args: &Args,
    baseline_document: &JsonValue,
) -> Result<(), TinyOneError> {
    let Some(metadata) = baseline_document.get("metadata") else {
        return Err(TinyOneError::Runtime(
            "Priority 5 baseline has no metadata".to_string(),
        ));
    };
    let Some(options) = metadata.get("benchmark_options") else {
        return Err(TinyOneError::Runtime(
            "Priority 5 baseline has no benchmark options".to_string(),
        ));
    };
    ...
    let expected_metadata = [
        ...
        (
            "machine label",
            "machine_label",
            json!(args.machine_label.as_deref()),
        ),
        (
            "power policy",
            "power_policy",
            json!(args.power_policy.as_deref()),
        ),
    ];
...
fn run() -> Result<i32, TinyOneError> {
    ...
    if args.priority_5_gate && !args.priority_5_only {
        return Err(TinyOneError::Runtime(
            "--priority-5-gate requires --priority-5-only".to_string(),
        ));
    }
    ...
    if args.save_baseline_auto {
        ...
        let path = automatic_baseline_path(
            document
                .get("metadata")
                .expect("benchmark document metadata"),
        );
...
mod tests {
    ...
    fn automatic_baselines_stay_under_target_perf() {
        ...
        assert_eq!(
            ...
            Some("baseline-1234567890ab-42.json")
        );
    }
    ...
    fn evidence_quality_rejects_skipped_checks_and_unstable_rows() {
        ...
        let quality =
            assess_current_evidence(&args, &[result("runtime.test", 100.0, 10.01)], false);
        ...
        assert!(
            quality
                .rejections
                .iter()
                .any(|reason| reason.contains("10.01% CV"))
        );
    }
    ...
    fn stored_evidence_requires_metadata_and_acceptable_variance() {
        ...
        let quality = assess_saved_evidence(
            &baseline,
            baseline_items(&baseline).expect("benchmark rows"),
        );
        assert!(!quality.decision_eligible);
        assert!(
            quality
        ...
        assert!(
            quality
                .rejections
                .iter()
                .any(|reason| reason.contains("12.46% CV"))
        );
    }
    ...
    fn priority_5_gate_requires_matching_capture_metadata() {
        ...
        require_priority_5_baseline_compatibility(&args, &baseline)
            .expect("matching guardrail metadata");
    ...
    fn default_loop_workload_reaches_the_quickened_tier() {
        ...
        let mut program = JitProgram::compile_verified(&fixture.verified)
            .expect("benchmark program should compile");
        run_compiled_jit(&mut program, Vec::new());
