...
impl std::fmt::Display for AllocTableError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            AllocTableError::GenerationMismatch { expected, actual } => write!(
                f,
...
impl AllocTable {
    ...
    pub fn remove(
        ...
        vm_address: usize,
        vm_generation: u64,
    ) -> Result<AllocRecord, AllocTableError> {
        let mut guard = self.inner.lock().unwrap();
        let record = guard
            .records
            .get(&vm_address)
            .ok_or(AllocTableError::NotFound)?;
...
mod tests {
    ...
    fn double_insert_fails() {
        ...
        assert_eq!(
            err,
            AllocTableError::AlreadyExists,
            "second insert of live slot should fail with AlreadyExists"
        );
    }
