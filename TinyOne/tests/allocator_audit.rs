...
fn alloc_table_stale_generation_rejected() {
    ...
    assert!(
        stale.is_none(),
        "stale generation lookup must return None; got: {stale:?}"
    );
...
fn alloc_table_double_free_detected() {
    ...
    assert!(
        result.is_err(),
        "second remove of the same address must return Err"
    );
...
fn memory_log_concurrent_log_no_deadlock() {
    ...
    for t in 0u64..16 {
        ...
        handles.push(thread::spawn(move || {
            for i in 0u64..1_000 {
                log.log(MemoryLogEntry::success(
                    t * 1_000 + i,
                    t,
...
fn hook_panicking_hook_does_not_propagate() {
    ...
    assert_eq!(
        counter.count(),
        1,
        "counting hook must be called even after prior hook panicked"
    );
}
...
fn tiny_allocator_stale_pointer_fires_hook() {
    ...
    assert!(
        !pusher.has_errors(),
        "no errors expected after a valid free; pusher: {:?}",
        pusher.drain_errors()
    );
    ...
    assert!(
        matches!(
            err,
            TinyAllocatorError::DoubleFree { .. } | TinyAllocatorError::GenerationMismatch { .. }
        ),
    ...
    assert!(
        pusher.has_errors(),
        "MemoryErrorPusher must report errors after the stale/double free"
    );
    ...
    assert!(
        found_error_event,
        "expected StalePointer/DoubleFree/AccessViolation event; got {events:?}"
    );
}
...
fn tiny_allocator_log_grows_bounded() {
    ...
    assert!(
        snap.len() <= 10,
        "log snapshot must not exceed log_capacity=10; got len={}",
        snap.len()
    );
}
...
fn tiny_allocator_post_shutdown_free_returns_not_found() {
    ...
    assert!(
        matches!(
            err,
            TinyAllocatorError::DoubleFree { .. } | TinyAllocatorError::NotFound { .. }
        ),
...
fn alloc_table_get_dead_record_still_returns_some() {
    ...
    assert!(
        record.is_some(),
        "get() on a dead (but not removed) record must return Some; got None"
    );
    assert!(
        !record.unwrap().live,
        "returned dead record must have live=false"
    );

    // all_live() must NOT include the dead record.
    assert!(
        table.all_live().is_empty(),
        "dead record must not appear in all_live()"
    );
}
...
fn memory_log_disable_suppresses_concurrent_writes() {
    ...
    assert_eq!(
        log.len(),
        0,
        "no entries must be written while logging is disabled; got len={}",
        log.len()
    );
...
fn alloc_table_stats_live_bytes_never_underflows() {
    ...
    for i in 0u64..20 {
        ...
        assert!(
            stats.live_bytes < usize::MAX / 2,
            "live_bytes appears to have wrapped; got {}",
            stats.live_bytes
        );
    }
