...
use crate::ralloc::{
    allocate_aligned, is_supported_native_alignment, ralloc_free, reallocate_aligned,
};
...
impl RallocBuffer {
    ...
    pub fn try_resize(&mut self, new_len: usize) -> Result<(), RallocError> {
        ...
        let resized = reallocate_aligned(self.ptr.as_ptr().cast::<c_void>(), new_len, self.align)
            .cast::<u8>();
