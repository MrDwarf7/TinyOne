...
use crate::block::{
    ALIGNMENT, BlockHeader, HEADER_SIZE, MIN_BLOCK_SIZE, align_up, request_to_block_size,
};
...
impl Arena {
    ...
    pub(crate) fn from_region(
        ...
        region::register(
            region::RegionRecord::new(start, len, slot, generation)
    ...
    pub(crate) fn allocate_aligned(
        ...
        while current < end {
            ...
            if !block.is_allocated() {
                ...
                self.stats
                    .record_allocate(allocated.size().saturating_sub(HEADER_SIZE));
                return Ok(payload);
            }
    ...
    fn aligned_allocation_slot(
        ...
        let mut header_addr =
            align_up(block_start.checked_add(HEADER_SIZE)?, align)?.checked_sub(HEADER_SIZE)?;
    ...
    fn coalesce(&mut self, header: *mut BlockHeader) {
        ...
        if let Some(previous) = self.previous_block(header) {
            ...
            if !previous_block.is_allocated()
                && let Some(size) =
                    BlockHeader::coalesced_size(previous_block.size(), current_block.size())
            {
...
mod tests {
    ...
    fn arena_coalesces_adjacent_free_blocks() {
        ...
        arena
            .deallocate(second)
            .expect("second free should succeed");
    ...
    fn arena_rejects_in_region_non_payload_pointers() {
        ...
        assert_eq!(
            arena.deallocate(non_payload),
            Err(super::ArenaError::InvalidPointer)
        );
        assert_eq!(
            arena.allocation_size(non_payload),
            Err(super::ArenaError::InvalidPointer)
        );
    ...
    fn arena_stats_track_current_and_peak_payload_bytes() {
        ...
        assert_eq!(
            arena.stats().current_payload_bytes(),
    ...
    fn arena_allocate_aligned_returns_requested_payload_alignment() {
        ...
        arena
            .deallocate(payload)
            .expect("aligned allocation should free");
    }
    ...
    fn integrity_snapshot_rejects_invalid_block_step() {
        ...
        assert_eq!(
            arena.integrity_snapshot(),
            Err(super::ArenaError::InvalidRegion)
        );
    }
}
