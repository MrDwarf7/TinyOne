...
use core::{cell::UnsafeCell, cmp, ffi::c_void, ptr};
...
static STORAGE: StaticArenaStorage =
    StaticArenaStorage([const { UnsafeCell::new([0; ARENA_BYTES]) }; ARENA_COUNT]);
static ARENAS: [SpinLock<ArenaState>; ARENA_COUNT] =
    [const { SpinLock::new(ArenaState::new()) }; ARENA_COUNT];
...
mod tests {
    ...
    use super::{
        ReallocCopyGuardSnapshot, ralloc_calloc, ralloc_free, ralloc_malloc, ralloc_realloc,
    };
    ...
    fn abi_allocates_from_additional_arena_when_primary_is_full() {
        ...
        assert_eq!(
            region::lookup(first.addr()).map(|record| record.owner()),
            Some(ArenaSlot::new(0))
        );
        assert_eq!(
            region::lookup(second.addr()).map(|record| record.owner()),
            Some(ArenaSlot::new(1))
        );
    }
    ...
    fn abi_test_hook_distinguishes_region_owner_from_live_realloc_source() {
        ...
        assert_eq!(
            region::snapshot_lookup_for_tests(ptr.addr()).map(|record| record.owner()),
            Some(ArenaSlot::new(0))
        );
        assert_eq!(
            super::realloc_copy_guard_snapshot_for_tests(ptr),
