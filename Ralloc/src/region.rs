...
mod tests {
    use super::{
        ArenaSlot, RegionRecord, RegionRegistry, clear_for_tests, lookup, register, remove,
    };
    ...
    fn registry_finds_valid_pointers_and_rejects_boundaries() {
        ...
        assert_eq!(
            registry.lookup(0x1000).map(|record| record.owner()),
            Some(owner)
        );
        assert_eq!(
            registry.lookup(0x10ff).map(|record| record.owner()),
            Some(owner)
        );
    ...
    fn registry_removes_only_exact_ranges() {
        ...
        assert_eq!(
            registry.remove(0x2000, 0x100),
            Err(super::RegionError::NotFound)
        );
        assert_eq!(registry.lookup(0x2000), Some(region));

        registry
            .remove(0x2000, 0x200)
            .expect("exact range should remove");
    ...
    fn global_registry_routes_registered_addresses() {
        ...
        assert_eq!(
            lookup(0x3070).map(|record| record.owner()),
            Some(ArenaSlot::new(4))
        );
