...
fn mixed_size_interleave_does_not_exhaust_arena() {
    ...
    for round in 0..16 {
        ...
        for (index, &n) in sizes.iter().enumerate().skip(1).step_by(2) {
            let mut buf =
                RallocBuffer::new(n).expect("re-allocation into freed holes should succeed");
...
fn over_aligned_free_reclaimed_by_normal_alloc() {
    ...
    let mut aligned =
        RallocBuffer::try_new_aligned(SIZE, ALIGN).expect("over-aligned allocation should succeed");
    ...
    let mut normal =
        RallocBuffer::new(SIZE).expect("normal alloc after aligned free should succeed");
    ...
    let mut second_aligned = RallocBuffer::try_new_aligned(SIZE, ALIGN)
        .expect("second over-aligned alloc should succeed");
    ...
    for align in [16usize, 32, 64, 128] {
        let buffer = RallocBuffer::try_new_aligned(32, align)
            .expect("arena should still satisfy aligned requests after frees");
...
fn repeated_same_size_cycle_does_not_fragment() {
    ...
    for i in 0..CYCLES {
        let mut buf = RallocBuffer::new(SIZE)
            .unwrap_or_else(|| panic!("allocation should succeed on cycle {i}"));
        let marker = i as u8;
        buf.as_mut_slice().fill(marker);
        assert!(buf.as_slice().iter().all(|&byte| byte == marker));
        drop(buf);
    ...
    let bufs: Vec<_> = (0..4)
        .map(|index| {
            let mut buffer =
                RallocBuffer::new(SIZE).expect("simultaneous alloc after cycles should succeed");
    ...
    for (index, buffer) in bufs.iter().enumerate() {
        assert!(
            buffer
                .as_slice()
                .iter()
                .all(|&byte| byte == (0xe0 | index as u8))
        );
    }
    drop(bufs);
}
