...
fn buffer_aligned_resize_preserves_alignment_and_bytes() {
    let mut buffer =
        RallocBuffer::try_new_aligned(8, 64).expect("aligned buffer allocation should succeed");
    ...
    buffer
        .try_resize(128)
        .expect("aligned resize should succeed");
...
fn buffer_rejects_invalid_alignment_requests() {
    assert_eq!(
        RallocBuffer::try_new_aligned(8, 3).err(),
        Some(RallocError::InvalidAlignment)
    );
    assert_eq!(
        RallocBuffer::try_new_aligned(8, 8192).err(),
        Some(RallocError::InvalidAlignment)
    );
}
...
fn buffer_try_new_reports_invalid_zero_and_oom_separately() {
    ...
    assert_eq!(
        RallocBuffer::try_new(usize::MAX).err(),
        Some(RallocError::OutOfMemory)
    );
}
...
fn buffer_raw_parts_round_trip_transfers_ownership_explicitly() {
    ...
    let recovered = unsafe { RallocBuffer::from_raw_parts(ptr, len) }
        .expect("raw parts should recover ownership");
