...
fn print_help() {
    println!(
        "usage: tinylang_bench [--quick] [--filter TEXT] [--repeats N] \\
...
fn build_benchmarks() -> Vec<Benchmark> {
    ...
    vec![
        ...
        bench("runtime.jit_straightline", 10_000, {
            let mut program = JitProgram::compile(&*straightline.program).expect("benchmark program should compile");
            move || run_compiled_jit(&mut program, Vec::new())
        }),
        bench("runtime.jit_loop_control", 2_000, {
            let mut program = JitProgram::compile(&*loop_fixture.program).expect("benchmark program should compile");
            move || run_compiled_jit(&mut program, Vec::new())
        }),
        bench("runtime.jit_function_calls", 600, {
            let mut program = JitProgram::compile(&*functions.program).expect("benchmark program should compile");
            move || run_compiled_jit(&mut program, Vec::new())
        }),
        bench("runtime.jit_control_interrupts", 2_000, {
            let mut program = JitProgram::compile(&*interrupts.program).expect("benchmark program should compile");
            move || run_compiled_jit(&mut program, Vec::new())
        }),
        bench("runtime.jit_heap_structs", 1_000, {
            let mut program = JitProgram::compile(&*heap.program).expect("benchmark program should compile");
            move || run_compiled_jit(&mut program, Vec::new())
        }),
        bench("runtime.jit_builtin_heavy", 2_000, {
            let mut program = JitProgram::compile(&*builtins.program).expect("benchmark program should compile");
            move || run_compiled_jit(&mut program, Vec::new())
        }),
...
fn print_table(results: &[BenchmarkResult]) {
    ...
    println!("\n{:<44} {:>7} {:>12} {:>12} {:>6}", "benchmark", "iters", "best/iter", "mean/iter", "cv%");
    println!("{}", "-".repeat(86));
    for result in results {
        let flag = if result.cv_pct() > CV_WARN { " !" } else { "  " };
        println!(
            "{:<44} {:>7} {:>12} {:>12} {:>5.1}%{}",
            result.name,
            result.iterations,
...
fn compare_to_baseline(results: &[BenchmarkResult], path: &str) -> Result<(), TinyOneError> {
    ...
    let Some(items) = baseline.as_array() else {
        return Err(TinyOneError::Runtime("Baseline must be a JSON list".to_string()));
    };
    ...
    for result in results {
        ...
        println!("{:<44} {:>12} {:>12} {:+8.1}%", result.name, format_duration(old), format_duration(new), delta);
    }
    Ok(())
}
...
fn run() -> Result<i32, TinyOneError> {
    ...
    println!("benchmarks={}  repeats={}  quick={}\n", benchmarks.len(), args.repeats, title_bool(args.quick));
