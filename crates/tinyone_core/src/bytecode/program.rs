...
impl ModuleCapabilities {
    ...
    pub(crate) fn from_names(names: &[String]) -> Result<Self> {
        ...
        for name in names {
            ...
            if bits & capability.bit() != 0 {
                return Err(TinyOneError::compile(format!("Module capability {name:?} is declared more than once")));
            }
            bits |= capability.bit();
        }
        Ok(Self(bits))
    ...
    pub(crate) fn from_bits(bits: u8) -> Result<Self> {
        ...
        if bits & !known != 0 {
            return Err(TinyOneError::compile("Module capabilities contain unknown bits"));
        }
        Ok(Self(bits))
    }
...
impl ModulePermissions {
    ...
    pub(crate) fn from_artifact(
        ...
        let expected = Self::from_detailed_fields(
            filesystem_read,
            filesystem_write,
            environment_read.as_ref().is_none_or(|items| !items.is_empty()),
            network_outbound,
            network_listen,
            process_spawn,
            ffi_allowed,
        ...
        if !expected.is_subset_of(capabilities) {
            return Err(TinyOneError::compile("Artifact module detailed permissions exceed its declared capabilities"));
        }
        Ok(Self {
            capabilities,
            filesystem_read,
...
impl Program {
    ...
    pub(crate) fn capabilities_for_function(&self, function_index: Option<usize>) -> ModulePermissions {
        ...
        if let Some(module) = self.modules.iter().find(|module| module.name == module_name) {
    ...
    pub(crate) fn needs_runtime_host_permissions(&self) -> bool {
        self.code
            .iter()
            .chain(self.functions.iter().flat_map(|function| function.code.iter()))
    ...
    fn function_module(&self, function_index: usize) -> Option<usize> {
        ...
        self.modules.iter().position(|module| module.name == module_name)
    }
...
impl Function {
    pub fn new(name: impl Into<String>, param_count: usize, code: Vec<Instr>, slot_count: usize) -> Self {
        Self {
            name: name.into(),
...
mod tests {
    ...
    fn cache_authority_scan_rejects_any_builtin_in_main_or_function() {
        ...
        let in_main = Program::new(vec![Instr::new(Op::Builtin, filesystem_builtin as i64, 1)], 0);
        assert!(in_main.needs_runtime_host_permissions());

        let otherwise_safe_builtin = crate::builtin_index("len").expect("length builtin");
        let in_main = Program::new(vec![Instr::new(Op::Builtin, otherwise_safe_builtin as i64, 1)], 0);
        assert!(in_main.needs_runtime_host_permissions());

        let unsafe_builtin = crate::builtin_index("free").expect("unsafe builtin");
        let in_function = Program::new(vec![Instr::new(Op::Halt, 0, 0)], 0).with_functions(vec![Function::new(
            "module.release",
            0,
