...
pub(crate) fn patch_module(
    ...
    let owned_name = |name: &str| name.split_once('.').is_some_and(|(owner, _)| owner == module_name);
    ...
    let function_map = replacement
        ...
        .map(|item| {
            function_indexes.get(&item.name).copied().ok_or_else(|| {
                TinyOneError::compile(format!("Incremental module requires unavailable function {:?}", item.name))
            })
        })
    ...
    let struct_map = replacement
        ...
        .map(|item| {
            struct_indexes.get(&item.name).copied().ok_or_else(|| {
                TinyOneError::compile(format!("Incremental module requires unavailable struct {:?}", item.name))
            })
        })
        .collect::<Result<Vec<_>>>()?;
    ...
    for (replacement_index, replacement_function) in replacement.functions.iter().enumerate() {
        ...
        remap_code(&mut function.code, &function_map, &string_map, &struct_map, &field_map, &enum_map)?;
    ...
    for import in &replacement_module.imports {
        if !program.modules.iter().any(|module| module.name == import.resolved) {
...
fn require_same_set<'a>(
    ...
    if old != new {
        return Err(TinyOneError::compile(format!("Incremental module {kind} declarations changed")));
    }
    Ok(())
}
...
fn remap_code(
    ...
    for instruction in code {
        ...
        let old_index = usize::try_from(instruction.arg)
            .map_err(|_| TinyOneError::compile("Incremental module contains a negative table index"))?;
        let new_index = mapping
            .get(old_index)
            .copied()
            .ok_or_else(|| TinyOneError::compile("Incremental module contains an invalid table index"))?;
        instruction.arg =
            i64::try_from(new_index).map_err(|_| TinyOneError::compile("Incremental table index overflow"))?;
    }
    Ok(())
}
