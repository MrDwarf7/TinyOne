...
pub(crate) fn resolve_import(from_filename: &str, import_path: &str) -> Result<(String, String)> {
    ...
    let source = fs::read_to_string(&path).map_err(|error| TinyOneError::compile(format!("Import error: {error}")))?;
...
fn resolve_manifest_import(base: &Path, import_path: &str) -> Result<Option<PathBuf>> {
    ...
    for directory in base.ancestors() {
        ...
        let modules = data.get("modules").and_then(JsonValue::as_object).ok_or_else(|| {
            TinyOneError::compile(format!("Package manifest {} must contain a modules object", manifest_path.display()))
        })?;
