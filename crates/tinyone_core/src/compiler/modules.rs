...
use crate::{CompilerSharedState, ModuleCapabilities, ModulePermissions, ProjectConfig, Result, TinyOneError};
...
impl ModuleResolver {
    ...
    pub(crate) fn new(root_file: &Path) -> Result<Self> {
        let root_file = root_file.canonicalize().map_err(|error| {
            TinyOneError::compile(format!("File error: could not canonicalize module root: {error}"))
        })?;
    ...
    pub(crate) fn resolve_import(&mut self, from_filename: &str, import_path: &str) -> Result<ResolvedModule> {
        ...
        {
            return Err(TinyOneError::compile(format!("Import {import_path:?} escapes the module sandbox")));
        }
        ...
        let base = if let Some(base) = self.base_dirs.get(from_filename) {
            ...
            self.base_dirs.insert(from_filename.to_string(), base.clone());
            base
        };
        ...
        let path = if let Some(path) = self.canonical_resolutions.get(&candidate) {
            ...
            self.canonical_resolutions.insert(candidate.clone(), path.clone());
            path
        };
        ...
        let source = if let Some(source) = self.sources.get(&path) {
            ...
            let source =
                read_source_file(&path).map_err(|error| TinyOneError::compile(format!("Import error: {error}")))?;
        ...
        let permissions = if let Some(verification) =
            self.config.verify_module_signature(import_path, &path, &source)?
        {
            if !verification.declared_capabilities.is_subset_of(configured_capabilities) {
                return Err(TinyOneError::compile(format!(
            ...
            self.signed_module_valid_until = Some(
                self.signed_module_valid_until
                    .map_or(verification.expires_at, |existing| existing.min(verification.expires_at)),
            );
        ...
        self.module_permissions.insert(path.clone(), permissions.clone());
    ...
    fn resolve_manifest_import(&mut self, base: &Path, import_path: &str) -> Result<Option<(PathBuf, ManifestModule)>> {
    ...
    fn read_manifest_modules(&mut self, manifest_path: &Path) -> Result<Option<&HashMap<String, ManifestModule>>> {
        if !self.manifests.contains_key(manifest_path) {
            if !manifest_path.exists() {
                ...
                let bytes = read_limited_file(&canonical_manifest, MAX_MANIFEST_BYTES, "Package manifest")?;
                ...
                let text = String::from_utf8(bytes)
                    .map_err(|error| TinyOneError::compile(format!("Package manifest must be UTF-8: {error}")))?;
                let data: JsonValue = serde_json::from_str(&text)
                    .map_err(|error| TinyOneError::compile(format!("Package manifest JSON error: {error}")))?;
                let modules = data.get("modules").and_then(JsonValue::as_object).ok_or_else(|| {
...
fn parse_manifest_module(value: &JsonValue, name: &str, manifest_path: &Path) -> Result<ManifestModule> {
    let (path, capability_values) = if let Some(path) = value.as_str() {
        ...
        let path = object.get("path").and_then(JsonValue::as_str).ok_or_else(|| {
            TinyOneError::compile(format!(
...
pub(crate) fn read_source_file(path: &Path) -> Result<String> {
    ...
    String::from_utf8(bytes).map_err(|error| TinyOneError::compile(format!("Source must be UTF-8: {error}")))
}
...
fn read_limited_file(path: &Path, max_bytes: usize, kind: &str) -> Result<Vec<u8>> {
    let mut file =
        fs::File::open(path).map_err(|error| TinyOneError::compile(format!("{kind} read error: {error}")))?;
...
pub(crate) fn unique_module_name(state: &mut CompilerSharedState, base_name: &str, filename: &str) -> String {
    if state
        .module_name_owners
        .get(base_name)
    ...
    state.module_name_owners.insert(name.clone(), filename.to_string());
    name
}
...
fn sanitize_identifier(text: &str) -> String {
    let mut out = text
        .chars()
        .map(|ch| if ch == '_' || ch.is_alphanumeric() { ch } else { '_' })
        .collect::<String>()
        .trim_matches('_')
        .to_string();
...
mod tests {
    ...
    impl TempDir {
        fn new() -> Self {
            ...
            let path = std::env::temp_dir().join(format!("tinyone-resolver-cache-{}-{stamp}", std::process::id()));
    ...
    fn session_cache_reuses_manifest_resolution_and_source_text() {
        ...
        fs::write(temp.0.join("tinyone.json"), r#"{"modules":{"first":"shared.to","second":"shared.to"}}"#)
        ...
        assert_eq!(1, resolver.manifests.values().filter(|item| item.is_some()).count());
