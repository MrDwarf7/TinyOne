...
fn run_direct_builtin(
    ...
    let result = match builtin {
        ...
        JitBuiltin::MapSet => {
            ...
            let target = jit_pop(stack)?;
            match key {
                Value::I64(key) => crate::runtime::stdlib::b_map_set_i64(context, &target, key, value)?,
...
impl<'a> JitVm<'a> {
    ...
    pub(crate) fn run_chunk(
        ...
        let result = self.run_chunk_with_stack(chunk_index, memory, stdout, global_memory, &mut stack);
        ...
        loop {
            let instr = {
                ...
                let Some(instr) = chunk.ops.get(pc).copied() else {
                    return Err(TinyOneError::runtime(format!("Invalid program counter in {}", chunk.name)));
                };
                instr
            };
            let op_pc = pc;
            ...
            match instr {
                ...
                JitOp::PushString(index) => {
                    let text = self
                        ...
                        .strings
                        .get(index)
                        .ok_or_else(|| TinyOneError::runtime(format!("Invalid string index {index}")))?
                        .clone();
                ...
                JitOp::CompareSlotIntJumpIfZeroHot(operands) => {
                    ...
                    let is_false = match memory.compare_int(slot, value, op)? {
                        ...
                        None => runtime_is_false(&runtime_compare_int(op, memory.load(slot)?, Value::I64(value))?),
                    };
                    if is_false {
                        pc = target;
                    }
                ...
                JitOp::ArrayLenSlotJumpIfZero(slot, target) => {
                    ...
                    let length = crate::runtime::builtins::runtime_len(&self.context, &target_value)?;
                ...
                JitOp::MapGetAddSlots(slots) => {
                    ...
                    let value = match key {
                        Value::I64(key) => crate::runtime::stdlib::b_map_get_i64(&mut self.context, &target, key)?,
                ...
                JitOp::MapSetMulSlots(slots) => {
                    ...
                    let value = match memory.mul_int(slots.value_slot(), slots.multiplier())? {
                        ...
                        None => runtime_mul_int(memory.load(slots.value_slot())?, Value::I64(slots.multiplier()))?,
                    };
                    let result = match key {
                        Value::I64(key) => {
                            crate::runtime::stdlib::b_map_set_i64(&mut self.context, &target, key, value)?
                        }
                        key => crate::runtime::stdlib::b_map_set(&mut self.context, &target, key, value)?,
                    };
                ...
                JitOp::Call(function_index, arg_count) => {
                    ...
                    let result = self.call_function(function_index, stack, arg_count, stdout, globals)?;
                    stack.push(result);
                }
                ...
                JitOp::GetField(field_index) => {
                    ...
                    let field = self
                        ...
                        .fields
                        .get(field_index)
                        .ok_or_else(|| TinyOneError::runtime(format!("Invalid field index {field_index}")))?;
    ...
    fn call_function_with_args(
        ...
        self.call_depth -= 1;
        result?.ok_or_else(|| {
            TinyOneError::runtime(format!("Function {:?} returned no value", self.function_name(function_index)))
        })
    }
