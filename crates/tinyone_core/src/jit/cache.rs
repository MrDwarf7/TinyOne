...
impl JitExecutionProfile {
    pub(crate) fn record(&mut self, op: JitOp, stack_depth: usize) {
        ...
        let entry = self.opcodes.entry(op.profile_name().to_owned()).or_default();
...
impl JitCache {
    ...
    fn compile_mut(&mut self, verified: &VerifiedProgram) -> Result<&mut JitProgram> {
        ...
        if !self.cache.contains_key(&key) {
            let compiled = JitProgram::compile_with_fingerprint(verified, key.clone(), self.options)?;
    ...
    fn compile_source_cached_with_digest(&mut self, source: &str, digest: [u8; 16]) -> Result<VerifiedProgram> {
        ...
        let cached = self.source_cache.get_mut(&digest).and_then(|entries| {
            entries.iter_mut().find(|entry| entry.source == source).map(|entry| {
    ...
    fn insert_source_cache(&mut self, digest: [u8; 16], source: &str, program: &VerifiedProgram, last_used: u64) {
        ...
        while self.source_cache_len() >= max_entries || self.source_cache_bytes.saturating_add(source_bytes) > max_bytes
        {
            if !self.evict_lru_source() {
                self.source_bypasses = self.source_bypasses.saturating_add(1);
                return;
        ...
        self.source_cache.entry(digest).or_default().push(SourceCacheEntry {
    ...
    fn evict_lru_source(&mut self) -> bool {
        ...
        let (removed_bytes, remove_bucket) = {
            let entries = self.source_cache.get_mut(&digest).expect("LRU source bucket exists");
...
mod tests {
    ...
    fn source_cache_limits_have_bounded_defaults_and_are_configurable() {
        ...
        assert_eq!(defaults.source_cache_max_entries(), DEFAULT_SOURCE_CACHE_MAX_ENTRIES);
        assert_eq!(defaults.source_cache_byte_limit(), DEFAULT_SOURCE_CACHE_MAX_BYTES);
    ...
    fn configurable_threshold_quickens_at_the_requested_back_edge() {
        ...
        let mut stdout = Vec::new();
        cache.run_verified_program(&program, &mut stdout, Vec::new()).unwrap();
    ...
    fn same_digest_bucket_still_requires_exact_source_identity() {
        ...
        assert_eq!(run_source_with_digest(&mut cache, "print 1", forced_collision), b"1\n");
        assert_eq!(run_source_with_digest(&mut cache, "print 2", forced_collision), b"2\n");
        assert_eq!(run_source_with_digest(&mut cache, "print 1", forced_collision), b"1\n");
    ...
    fn entry_limit_evicts_lru_source_and_recompiles_on_next_use() {
        ...
        for (source, expected) in [
            ...
            let mut stdout = Vec::new();
            cache.run_source(source, &mut stdout, Vec::new()).expect("source runs");
            assert_eq!(stdout, expected);
        }
