...
pub(crate) fn runtime_call_builtin(
    ...
    match builtin.name {
        ...
        "array" => {
            ...
            Ok(Value::Heap(context.heap().alloc_array(vec![args[1].clone(); count])?))
        }
        ...
        "read" => {
            ...
            if looks_like_int(&raw) {
                Ok(Value::I64(
                    raw.parse()
                        .map_err(|_| TinyOneError::runtime("read() integer input is out of range"))?,
                ))
            } else {
        ...
        "read_int" => {
            let raw = context.read_raw()?;
            if !looks_like_int(&raw) {
                return Err(TinyOneError::runtime(format!("read_int() expected integer input, got {raw:?}")));
            }
            Ok(Value::I64(
                raw.parse()
                    .map_err(|_| TinyOneError::runtime("read_int() integer input is out of range"))?,
            ))
        }
        "read_str" => {
            let raw = context.read_raw()?;
            ...
            match &args[0] {
                ...
                _ => {
                    ...
                    if !looks_like_int(&text) {
                        return Err(TinyOneError::runtime("to_int() expects an integer or numeric string"));
                    }
                    Ok(Value::I64(
                        text.parse()
                            .map_err(|_| TinyOneError::runtime("to_int() integer input is out of range"))?,
                    ))
                }
            }
        }
        ...
        "ptr_ne" => {
            match runtime_pointer_eq(context, &args[0], &args[1])? {
