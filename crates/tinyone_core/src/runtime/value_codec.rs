...
pub(crate) fn add_i64_in_place(bytes: &mut [u8; ENCODED_VALUE_BYTES], value: i64) -> Result<bool> {
    ...
    let next = checked_i64_result(i128::from(read_i64(bytes, OFF_SCALAR)) + i128::from(value), "Addition")?;
...
pub(crate) fn sub_i64_in_place(bytes: &mut [u8; ENCODED_VALUE_BYTES], value: i64) -> Result<bool> {
    ...
    let next = checked_i64_result(i128::from(read_i64(bytes, OFF_SCALAR)) - i128::from(value), "Subtraction")?;
    write_i64(bytes, OFF_SCALAR, next);
    Ok(true)
}

pub(crate) fn compare_i64(bytes: &[u8; ENCODED_VALUE_BYTES], value: i64, op: Op) -> Result<Option<bool>> {
    if bytes[0] != TAG_I64 {
        return Ok(None);
    }
...
pub(crate) fn div_i64_in_place(bytes: &mut [u8; ENCODED_VALUE_BYTES], value: i64) -> Result<bool> {
    ...
    let next =
        floor_div(read_i64(bytes, OFF_SCALAR), value).ok_or_else(|| TinyOneError::runtime("Division overflow"))?;
    write_i64(bytes, OFF_SCALAR, next);
    Ok(true)
}
...
pub(crate) fn encoded_pointer_base(bytes: &[u8; ENCODED_VALUE_BYTES]) -> Option<(usize, u64, PointerKind)> {
    if bytes[0] != TAG_POINTER {
        return None;
    }
    let kind = PointerKind::from_u8(bytes[OFF_POINTER_KIND])?;
    Some((read_u64(bytes, OFF_ADDRESS) as usize, read_u64(bytes, OFF_GENERATION), kind))
}
...
fn decode_pointer(bytes: &[u8; ENCODED_VALUE_BYTES]) -> RawPointer {
    ...
    let cast = CastKind::from_u8(bytes[OFF_CAST_KIND]).expect("decode_pointer: invalid CastKind byte in encoded Value");
...
pub(crate) fn encode_value(value: &Value) -> Result<[u8; ENCODED_VALUE_BYTES]> {
    ...
    match value {
        ...
        Value::Heap(HeapRef { address, generation }) => {
            out[0] = TAG_HEAP;
...
pub(crate) fn decode_value(bytes: &[u8; ENCODED_VALUE_BYTES]) -> Value {
    ...
    match bytes[0] {
        ...
        TAG_I16 => Value::I16(i16::from_le_bytes(bytes[OFF_SCALAR..OFF_SCALAR + 2].try_into().unwrap())),
        TAG_I32 => Value::I32(i32::from_le_bytes(bytes[OFF_SCALAR..OFF_SCALAR + 4].try_into().unwrap())),
        TAG_I64 => Value::I64(read_i64(bytes, OFF_SCALAR)),
        TAG_U8 => Value::U8(bytes[OFF_SCALAR]),
        TAG_U16 => Value::U16(u16::from_le_bytes(bytes[OFF_SCALAR..OFF_SCALAR + 2].try_into().unwrap())),
        TAG_U32 => Value::U32(u32::from_le_bytes(bytes[OFF_SCALAR..OFF_SCALAR + 4].try_into().unwrap())),
        ...
        TAG_UNIT => Value::Unit,
        TAG_NULL => Value::Null,
        TAG_FUNCTION => Value::Function(u32::from_le_bytes(bytes[OFF_SCALAR..OFF_SCALAR + 4].try_into().unwrap())),
...
mod tests {
    ...
    fn round_trip(value: Value) {
        ...
        assert_eq!(decoded, value, "round trip mismatch for {value:?} (decoded: {decoded:?})");
    }
    ...
    fn round_trips_pointer_and_reference_variants() {
        round_trip(Value::Pointer(RawPointer::new(10, PointerKind::Array, 3, "", 2, CastKind::I32)));
        round_trip(Value::Pointer(RawPointer::new(10, PointerKind::Field, 0, "short_name", 2, CastKind::None)));
        round_trip(Value::Reference(RawPointer::new(1, PointerKind::Null, 0, "", 0, CastKind::None)));
    }

    #[test]
    fn pointer_field_name_at_exact_cap_round_trips() {
        let name = "x".repeat(MAX_INLINE_FIELD_BYTES);
        round_trip(Value::Pointer(RawPointer::new(1, PointerKind::Field, 0, name, 1, CastKind::None)));
    }

    #[test]
    fn pointer_field_name_over_cap_is_a_clean_error_not_a_panic() {
        let name = "x".repeat(MAX_INLINE_FIELD_BYTES + 1);
        let value = Value::Pointer(RawPointer::new(1, PointerKind::Field, 0, name, 1, CastKind::None));
