...
impl TinyHeap {
        ...
        if let Some(address) = self.free.pop() {
            let generation = {
                let generation = self
                    .generations
                    .get_mut(address)
                    .ok_or_else(|| TinyOneError::runtime(format!("Invalid heap free slot {address}")))?;
            ...
            *target = Some(object);
            ...
            Ok(HeapRef { address, generation })
        } else {
            if self.objects.len() >= MAX_HEAP_OBJECTS {
                return Err(TinyOneError::runtime(format!("Heap object limit {MAX_HEAP_OBJECTS} exceeded")));
            }
    ...
    pub(crate) fn ensure_can_allocate(&self, bytes: usize) -> Result<()> {
        ...
        let next_bytes = checked_or(self.stats.live_bytes.checked_add(bytes), "Heap byte accounting overflow")?;
        if next_bytes > MAX_HEAP_BYTES {
            return Err(TinyOneError::runtime(format!("Heap byte limit {MAX_HEAP_BYTES} exceeded")));
        }
        Ok(())
    }
    ...
    pub(crate) fn record_alloc(&mut self, bytes: usize) -> Result<()> {
        self.stats.live_objects =
            checked_or(self.stats.live_objects.checked_add(1), "Heap object accounting overflow")?;
        self.stats.live_bytes = checked_or(self.stats.live_bytes.checked_add(bytes), "Heap byte accounting overflow")?;
        self.stats.total_allocations =
            checked_or(self.stats.total_allocations.checked_add(1), "Heap allocation counter overflow")?;
    ...
    pub(crate) fn record_free(&mut self, bytes: usize) -> Result<()> {
        self.stats.live_objects =
            checked_or(self.stats.live_objects.checked_sub(1), "Heap object accounting underflow")?;
        self.stats.live_bytes = checked_or(self.stats.live_bytes.checked_sub(bytes), "Heap byte accounting underflow")?;
        self.stats.total_frees = checked_or(self.stats.total_frees.checked_add(1), "Heap free counter overflow")?;
        Ok(())
    }
    ...
    pub(crate) fn grow_array(&mut self, target: &Value, value: Value) -> Result<usize> {
        ...
        let HeapData::Array(values) = &object.data else {
            return Err(TinyOneError::runtime(format!("push() expects an array, got {}", object.kind())));
        };
        if values.len() >= MAX_ARRAY_LENGTH {
            return Err(TinyOneError::runtime(format!("push() exceeds maximum length {MAX_ARRAY_LENGTH}")));
        }
        ...
        let len = {
            ...
            let HeapData::Array(values) = &mut object.data else {
                return Err(TinyOneError::runtime("push() target stopped being an array"));
            };
            values.push(&encoded)?;
            values.len()
        };
    ...
    pub(crate) fn shrink_array(&mut self, target: &Value) -> Result<Value> {
        ...
        let HeapData::Array(_) = &object.data else {
            return Err(TinyOneError::runtime(format!("pop() expects an array, got {}", object.kind())));
        };
        let value = {
            // The generation and variant were validated above while this
            // exclusive heap borrow remained live, so no occupant can change.
            ...
            values
                .pop_with(|encoded| value_codec::decode_value(encoded.try_into().expect("encoded value stride")))
    ...
    pub(crate) fn alloc_buffer(&mut self, size: usize) -> Result<HeapRef> {
        let bytes =
            RallocBytes::zeroed(size).map_err(|e| TinyOneError::runtime(format!("Failed to allocate buffer: {e}")))?;
    ...
    pub(crate) fn get_address(&self, address: usize, generation: u64) -> Result<&HeapObject> {
        ...
        if generation != 0 && current_generation != generation {
            return Err(TinyOneError::runtime(format!("Stale heap pointer {address}")));
        }
        Ok(obj)
    }

    pub(crate) fn get_address_mut(&mut self, address: usize, generation: u64) -> Result<&mut HeapObject> {
        self.current_object(address)?;
        let current_generation = self
            .generations
            .get(address)
        ...
        self.objects
            .get_mut(address)
            .and_then(Option::as_mut)
            .ok_or_else(|| TinyOneError::runtime(format!("Use after free for heap pointer {address}")))
    }

    pub(crate) fn current_object(&self, address: usize) -> Result<&HeapObject> {
        if address == 0 || address >= self.objects.len() {
            return Err(TinyOneError::runtime(format!("Invalid heap pointer {address}")));
        }
        self.objects
            .get(address)
    ...
    pub(crate) fn free(&mut self, value: &Value) -> Result<()> {
        ...
        let target = self
            .objects
            .get_mut(reference.address)
            .ok_or_else(|| TinyOneError::runtime(format!("Invalid heap pointer {}", reference.address)))?;
        let object = target
            .take()
            .ok_or_else(|| TinyOneError::runtime(format!("Use after free for heap pointer {}", reference.address)))?;
...
pub(crate) fn heap_object_bytes(object: &HeapObject) -> usize {
    match &object.data {
        ...
        HeapData::Buffer(data) | HeapData::Alloc { data, .. } => data.len(),
        HeapData::Struct(record) | HeapData::Enum(record) => object.type_name.len() + record.byte_len(),
        HeapData::Cell(bytes) | HeapData::CharBuffer(bytes) | HeapData::Box(bytes) => bytes.len(),
...
fn alloc_value_slot(value: &Value) -> Result<RallocBytes> {
    let encoded = value_codec::encode_value(value)?;
    RallocBytes::from_slice(&encoded).map_err(|e| TinyOneError::runtime(format!("Failed to allocate value: {e}")))
}
