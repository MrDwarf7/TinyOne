...
pub(crate) fn expect_pointer(value: &Value, operation: &str) -> Result<RawPointer> {
    match value {
        ...
        _ => Err(TinyOneError::runtime(format!("{operation} expects a raw pointer"))),
    }
}

pub(crate) fn validate_pointer_base(context: &TinyRuntimeContext, pointer: &RawPointer, operation: &str) -> Result<()> {
    ...
    match pointer.kind.as_str() {
        ...
        _ => Err(TinyOneError::runtime(format!("{operation} got unknown raw pointer kind {:?}", pointer.kind))),
    }
}
...
fn pointer_identity(pointer: &RawPointer) -> (usize, u64, String, i64, String) {
    ...
    (pointer.address, pointer.generation, pointer.kind.clone(), pointer.index, pointer.field.clone())
}
...
pub(crate) fn runtime_make_pointer(context: &TinyRuntimeContext, args: &[Value]) -> Result<Value> {
    if args.len() == 1 {
        match &args[0] {
            ...
            _ => {
                return Err(TinyOneError::runtime("ptr() expects a heap value or pointer"));
            }
        }
    }
    let target = &args[0];
    ...
    let Value::Heap(reference) = target else {
        return Err(TinyOneError::runtime("ptr(value, index) expects an array or buffer heap value"));
    };
    let kind = context.heap().get(target)?.kind();
    match kind {
        "array" | "buffer" => {
            Ok(Value::Pointer(RawPointer::new(reference.address, kind, index, "", reference.generation, "")))
        }
        _ => Err(TinyOneError::runtime("ptr(value, index) expects an array or buffer heap value")),
    }
}
...
pub(crate) fn runtime_make_field_pointer(
    ...
    let Value::Heap(reference) = target else {
        return Err(TinyOneError::runtime("fieldptr() expects a struct heap value"));
    };
    ...
    if !field_names.iter().any(|name| name == &field) {
        return Err(TinyOneError::runtime(format!("Unknown field {field:?} on struct {:?}", type_name)));
    }
    Ok(Value::Pointer(RawPointer::new(reference.address, "field", 0, field, reference.generation, "")))
}

pub(crate) fn runtime_pointer_address(context: &TinyRuntimeContext, value: &Value) -> Result<Value> {
    match value {
        ...
        _ => Err(TinyOneError::runtime("ptr_addr() expects a heap value or raw pointer")),
    }
}
...
pub(crate) fn runtime_pointer_at(context: &TinyRuntimeContext, address: &Value) -> Result<Value> {
    ...
    Ok(Value::Pointer(RawPointer::new(address, "object", 0, "", generation, "")))
}

pub(crate) fn runtime_pointer_add(context: &TinyRuntimeContext, pointer: &Value, offset: &Value) -> Result<Value> {
    let pointer = expect_pointer(pointer, "ptr_add")?;
    validate_pointer_base(context, &pointer, "ptr_add")?;
    if pointer.kind == "null" && pointer.address == 0 {
        return Err(TinyOneError::runtime("Cannot apply pointer arithmetic to null"));
    }
    let offset = expect_int(offset, "ptr_add")?;
    match pointer.kind.as_str() {
        "object" => {
            if offset != 0 {
                return Err(TinyOneError::runtime("Object pointer arithmetic requires an array or buffer pointer"));
            }
        ...
        "field" => Err(TinyOneError::runtime("Cannot apply pointer arithmetic to field pointers")),
    ...
    match pointer.kind.as_str() {
        ...
        "array" => {
            ...
            let HeapData::Array(values) = &object.data else {
                return Err(TinyOneError::runtime("Array pointer no longer points at an array"));
            };
        ...
        "buffer" => Err(TinyOneError::runtime("Use read8/read16/read32 for buffer pointers")),
        "field" => {
            let heap = context.heap();
            let object = heap.get_address(pointer.address, pointer.generation)?;
            let HeapData::Struct(fields) = &object.data else {
                return Err(TinyOneError::runtime("Field pointer no longer points at a struct"));
            };
            fields
                .iter()
                ...
                .ok_or_else(|| {
                    TinyOneError::runtime(format!("Unknown field {:?} on struct {:?}", pointer.field, object.type_name))
                })
        }
        _ => Err(TinyOneError::runtime(format!("Unknown raw pointer kind {:?}", pointer.kind))),
    }
...
pub(crate) fn runtime_pointer_store(context: &mut TinyRuntimeContext, pointer: &Value, value: Value) -> Result<Value> {
    ...
    match pointer.kind.as_str() {
        ...
        "buffer" => Err(TinyOneError::runtime("Use write8/write16/write32 for buffer pointers")),
        "field" => {
            let mut heap = context.heap();
            let object = heap.get_address_mut(pointer.address, pointer.generation)?;
            ...
            if let Some((_, field_value)) = fields.iter_mut().find(|(name, _)| name == &pointer.field) {
...
pub(crate) fn runtime_pointer_type(context: &mut TinyRuntimeContext, pointer: &Value) -> Result<Value> {
...
pub(crate) fn runtime_pointer_offset(context: &TinyRuntimeContext, pointer: &Value) -> Result<Value> {
    let pointer = expect_pointer(pointer, "ptr_offset")?;
    validate_pointer_base(context, &pointer, "ptr_offset")?;
    Ok(Value::I64(if pointer.kind == "array" || pointer.kind == "buffer" {
        pointer.index
    } else {
        0
    }))
...
pub(crate) fn runtime_pointer_kind(context: &mut TinyRuntimeContext, pointer: &Value) -> Result<Value> {
...
pub(crate) fn runtime_pointer_field(context: &mut TinyRuntimeContext, pointer: &Value) -> Result<Value> {
...
pub(crate) fn runtime_pointer_eq(context: &TinyRuntimeContext, lhs: &Value, rhs: &Value) -> Result<Value> {
...
pub(crate) fn runtime_cast_pointer(context: &TinyRuntimeContext, pointer: &Value, type_value: &Value) -> Result<Value> {
    ...
    match type_name.as_str() {
        ...
        _ => Err(TinyOneError::runtime(format!("Unsupported pointer cast {type_name:?}"))),
    }
}

pub(crate) fn runtime_make_buffer(context: &mut TinyRuntimeContext, size: &Value) -> Result<Value> {
    let size = checked_bounded_len(expect_int(size, "buffer")?, "buffer() size", MAX_BUFFER_BYTES)?;
...
pub(crate) fn runtime_read_uint(
    ...
    if pointer.kind == "null" && pointer.address == 0 {
        return Err(TinyOneError::runtime(format!("{operation} cannot use null")));
    }
    if pointer.kind != "buffer" {
        return Err(TinyOneError::runtime(format!("{operation} expects a buffer pointer")));
    }
    let heap = context.heap();
    let object = heap.get_address(pointer.address, pointer.generation)?;
    let HeapData::Buffer(data) = &object.data else {
        return Err(TinyOneError::runtime("Buffer pointer no longer points at a buffer"));
    };
    ...
    Ok(match width {
        ...
        4 => Value::U32(value),
        _ => {
            return Err(TinyOneError::runtime(format!("{operation} unsupported integer width {width}")));
        }
    })
}
...
pub(crate) fn runtime_write_uint(
    ...
    if value_int < 0 || value_int > max_value {
        return Err(TinyOneError::runtime(format!("{operation} value must be in range 0..{max_value}")));
    }
    let pointer = expect_pointer(pointer, operation)?;
    if pointer.kind == "null" && pointer.address == 0 {
        return Err(TinyOneError::runtime(format!("{operation} cannot use null")));
