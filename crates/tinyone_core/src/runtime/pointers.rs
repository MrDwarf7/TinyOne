...
pub(crate) fn expect_pointer(value: &Value, operation: &str) -> Result<RawPointer> {
    match value {
        ...
        Value::Null => Ok(RawPointer::new(0, PointerKind::Null, 0, "", 0, CastKind::None)),
...
pub(crate) fn validate_pointer_base(context: &TinyRuntimeContext, pointer: &RawPointer, operation: &str) -> Result<()> {
    ...
    match pointer.kind {
        ...
        PointerKind::Null => {
            Err(TinyOneError::runtime(format!("{operation} got unknown raw pointer kind {:?}", pointer.kind)))
        }
    }
}
...
fn pointer_identity(pointer: &RawPointer) -> (usize, u64, PointerKind, i64, String) {
    ...
    (pointer.address, pointer.generation, pointer.kind, pointer.index, pointer.field.clone())
}
...
pub(crate) fn runtime_make_field_pointer(
    ...
    if !record.contains(&field) {
        return Err(TinyOneError::runtime(format!("Unknown field {field:?} on struct {:?}", object.type_name)));
    }
    Ok(Value::Pointer(RawPointer::new(
        reference.address,
        PointerKind::Field,
...
pub(crate) fn runtime_pointer_at(context: &TinyRuntimeContext, address: &Value) -> Result<Value> {
    ...
    Ok(Value::Pointer(RawPointer::new(address, PointerKind::Object, 0, "", generation, CastKind::None)))
}
...
pub(crate) fn runtime_pointer_add(context: &TinyRuntimeContext, pointer: &Value, offset: &Value) -> Result<Value> {
    ...
    match pointer.kind {
        ...
        PointerKind::Field => Err(TinyOneError::runtime("Cannot apply pointer arithmetic to field pointers")),
...
pub(crate) fn runtime_pointer_load(context: &TinyRuntimeContext, pointer: &Value) -> Result<Value> {
    ...
    match pointer.kind {
        ...
        PointerKind::Buffer => Err(TinyOneError::runtime("Use read8/read16/read32 for buffer pointers")),
        PointerKind::Field => {
            let heap = context.heap();
            let object = heap.get_address(pointer.address, pointer.generation)?;
            let HeapData::Struct(record) = &object.data else {
...
pub(crate) fn runtime_pointer_store(context: &mut TinyRuntimeContext, pointer: &Value, value: Value) -> Result<Value> {
    ...
    match pointer.kind {
        ...
        PointerKind::Buffer => Err(TinyOneError::runtime("Use write8/write16/write32 for buffer pointers")),
        PointerKind::Field => {
            let mut heap = context.heap();
            let object = heap.get_address_mut(pointer.address, pointer.generation)?;
...
pub(crate) fn runtime_pointer_offset(context: &TinyRuntimeContext, pointer: &Value) -> Result<Value> {
    ...
    Ok(Value::I64(if pointer.kind == PointerKind::Array || pointer.kind == PointerKind::Buffer {
        pointer.index
    } else {
        0
    }))
...
pub(crate) fn runtime_pointer_kind(context: &mut TinyRuntimeContext, pointer: &Value) -> Result<Value> {
    ...
    Ok(Value::Heap(context.heap().alloc_string(pointer.kind.as_str())?))
}
...
pub(crate) fn runtime_cast_pointer(context: &TinyRuntimeContext, pointer: &Value, type_value: &Value) -> Result<Value> {
    ...
    match CastKind::parse(&type_name) {
        ...
        None => Err(TinyOneError::runtime(format!("Unsupported pointer cast {type_name:?}"))),
    }
}
