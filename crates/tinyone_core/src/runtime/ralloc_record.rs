...
impl RallocRecord {
    ...
    pub(crate) fn new(tag: u32, variant: &str, fields: &[(String, Value)]) -> Result<Self> {
        ...
        for (name, _) in fields {
            ...
            if len > u16::MAX as usize {
                return Err(TinyOneError::runtime(format!("field name {name:?} too long")));
            }
        ...
        let total_len = header_len + variant_bytes.len() + total_name_bytes + field_count * ENCODED_VALUE_BYTES;

        let mut buf = vec![0u8; total_len];
        buf[OFF_TAG..OFF_TAG + 4].copy_from_slice(&tag.to_le_bytes());
        buf[OFF_VARIANT_LEN..OFF_VARIANT_LEN + 2].copy_from_slice(&(variant_bytes.len() as u16).to_le_bytes());
        buf[OFF_FIELD_COUNT..OFF_FIELD_COUNT + 2].copy_from_slice(&(field_count as u16).to_le_bytes());
    ...
    pub(crate) fn tag(&self) -> u32 {
        u32::from_le_bytes(self.bytes.as_slice()[OFF_TAG..OFF_TAG + 4].try_into().unwrap())
    }
...
mod tests {
    ...
    fn struct_get_set_and_fields_round_trip() {
        let fields = vec![("x".to_string(), Value::I64(1)), ("y".to_string(), Value::I64(2))];
        ...
        assert_eq!(record.fields(), vec![("x".to_string(), Value::I64(1)), ("y".to_string(), Value::I64(99)),]);
    }
    ...
    fn field_values_can_be_pointers_and_heap_refs() {
        let fields = vec![(
            "p".to_string(),
            Value::Pointer(crate::RawPointer::new(5, PointerKind::Array, 2, "", 1, CastKind::None)),
        )];
