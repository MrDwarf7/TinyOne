...
pub fn compile_source_with_filename(source: &str, filename: &str) -> Result<Arc<Program>> {
    ...
    let program = PeepholeOptimizer::optimize(Arc::try_unwrap(program).unwrap_or_else(|arc| (*arc).clone()));
...
pub fn compile_file(path: impl AsRef<Path>) -> Result<Arc<Program>> {
    ...
    let source = fs::read_to_string(&path).map_err(|error| TinyOneError::compile(format!("File error: {error}")))?;
    let shared = Rc::new(RefCell::new(CompilerSharedState::default()));
    let mut compiler =
        Compiler::new(source, path.to_string_lossy().to_string(), Some(resolve_import), false, "", shared)?;
    let program = compiler.compile()?;
