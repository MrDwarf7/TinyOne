    ...
    while let Some(arg) = iter.next() {
        match arg.as_str() {
            ...
            "--jit-threshold" => {
                ...
                args.jit_threshold = value
                    .parse::<u16>()
                    .map_err(|_| "--jit-threshold must be an integer from 0 to 65535".to_string())?;
            }
...
pub(crate) fn run() -> Result<i32, TinyOneError> {
    ...
    if !args.check {
        ...
        run_verified_program_with_jit_options(&program, &args.mode, &mut stdout, args.inputs, jit_options)?;
    }
    Ok(0)
}
...
mod tests {
    ...
    fn ergonomic_execution_and_optimization_flags_parse() {
        let args = parse(&["tinylang", "--vm", "-O0", "--jit-threshold", "3", "program.to"]).unwrap();
