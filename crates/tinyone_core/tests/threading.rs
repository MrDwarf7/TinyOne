...
"#;
    for mode in ["vm", "jit"] {
        ...
        assert_eq!(lines.last(), Some(&"3"), "expected last line to be 3 in {mode}, got: {s:?}");
        // All lines: the two thread prints and the final sum
        assert_eq!(lines.len(), 3, "expected 3 lines total in {mode}, got: {s:?}");
    }
}
...
"#;
    for mode in ["vm", "jit"] {
        ...
        assert!(msg.contains("already joined"), "expected 'already joined' in {mode}: {msg}");
    }
}
...
"#;
    for mode in ["vm", "jit"] {
        ...
        assert!(result.is_err());
        let msg = result.unwrap_err().to_string();
        assert!(msg.contains("expects 2"), "expected arity error in {mode}: {msg}");
    }
}
...
fn thread_spawn_unknown_function_errors() {
    ...
    for mode in ["vm", "jit"] {
        ...
        assert!(result.is_err());
        let msg = result.unwrap_err().to_string();
        assert!(msg.contains("not found"), "expected 'not found' in {mode}: {msg}");
    }
}
...
"#;
    ...
    for mode in ["vm", "jit"] {
        ...
        run_program_with_env(Arc::clone(&program), mode, &mut out, Vec::new(), vec!["argument".to_string()], env)
            .unwrap();
