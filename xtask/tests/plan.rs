...
fn command_plans_use_current_repo_paths() {
    ...
    assert!(check.iter().any(|cmd| cmd.contains("TinyOne/Cargo.toml")));
    assert!(check.iter().any(|cmd| cmd.contains("Ralloc/Cargo.toml")));

    let hooks = plan_for(Task::TestHooks).render_commands();
    assert!(
        hooks
            .iter()
            .any(|cmd| cmd.contains("--features testing-hooks"))
    );
    ...
    assert!(
        tools
            .iter()
            .any(|cmd| cmd.contains("Tools/abi_manifest.py"))
    );
