...
pub fn repo_root_from_cwd() -> Result<PathBuf, String> {
    ...
    if cwd.join("TinyOne/Cargo.toml").is_file()
        && cwd.join("Ralloc/Cargo.toml").is_file()
    ...
    Err(format!(
        "run xtask from the TinyOne repo root; missing TinyOne/Cargo.toml, Ralloc/Cargo.toml, or Tools/test_zip.py under {}",
        cwd.display()
    ))
}
...
pub fn usage() -> String {
    let tasks = Task::ALL
        ...
        .join("|");
    format!(
        "usage: cargo run --manifest-path xtask/Cargo.toml -- <{tasks}> [--dry-run]\n\
         \n\
         CI gate: cargo run --manifest-path xtask/Cargo.toml -- release-gate"
    )
}
